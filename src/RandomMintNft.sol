...
contract RandomMintNft is ERC721, Ownable {
    ...
    address private _vrfV2Cosumer;
    uint256[] public randomNums;

    constructor(string memory name_, string memory symbol_, string memory unrevealedTokenURI_, string memory tokenURI_, address vrfV2Cosumer_) ERC721(name_, symbol_) Ownable(msg.sender) {
        _tokenURI = tokenURI_;
        _unrevealedTokenURI = unrevealedTokenURI_;
        _vrfV2Cosumer = vrfV2Cosumer_;
    ...
    function setRandomNums(uint256 _requestID) public onlyOwner {
        (bool success, bytes memory data) = _vrfV2Cosumer.call(abi.encodeWithSignature("getRequestStatus(uint256)", _requestID));
        require(success, "_vrfV2Cosumer.getRequestStatus() failed");
